...
//            @RequestParam("size") int size)  // Size of the page (number of items per page)
//    {
//        Pageable pageable = PageRequest.of(page, size);
//        Page<PretPers> pretPersPage = pretPersRepository.getPretEnCours(soc, pageable);
 ...
 public ResponseEntity<List<PretPers>> getPretEnCours()
{
    ...
    public ResponseEntity<List<LigPret>> getLigPret(
            @RequestParam String soc,@RequestParam String mat,@RequestParam String pret)  // Size of the page (number of items per page)
    {
    ...
    public ResponseEntity<List<LigPretProjection>> getLigRegManuelle(
            @RequestParam String soc,@RequestParam String mois,@RequestParam String filtre,@RequestParam String corps,@RequestParam String grpPret)  // Size of the page (number of items per page)
    {
    ...
    public ResponseEntity<List<PiecePretPers>> getPiecePret( @PathVariable String soc,@PathVariable String mat,@PathVariable String pret)  // Size of the page (number of items per page)
    {
    ...
    ReponseDatDeb verifDatDeb(@RequestParam String codSoc, @RequestParam Long wprtEch, @RequestParam String prt_dat_acc, @RequestParam String  wdatDeb, @RequestParam String wdatRetr)
    {
         return creditService.verif_dat_deb(codSoc,wprtEch,prt_dat_acc,wdatDeb,wdatRetr);
    }
    @GetMapping("/verifDatAcc")
    ReponseDatAcc verifDatAcc(@RequestParam String codSoc, @RequestParam String mat_, @RequestParam String wcodGrpPret, @RequestParam String  wtypPret, @RequestParam String wdatEmb, @RequestParam String  wDatAcc)
    {
    ...
    ReponseCalculDetailCredit getDetailCredit(@RequestParam String wcodSoc,@RequestParam String wdatfin,@RequestParam String wdatretr,@RequestParam Double wprtmntglb,@RequestParam Double wprtech,@RequestParam Double wprttaux,@RequestParam BigDecimal wmntreport,@RequestParam String codGrpPret)
    {
         return creditService.calcul_detail_credit(wcodSoc,wdatfin,wdatretr,wprtmntglb,wprtech,wprttaux,wmntreport,codGrpPret);
    }

    @GetMapping("/verifPretCours")
    VerifPretResponse verifPretCours(@RequestParam String wcodSoc, @RequestParam String wmatPers, @RequestParam String wcodGrpPret, @RequestParam String wtypPret, @RequestParam String prtDatDeb)
    {
    ...
    {
         creditService.insert_remb_tranch(wcodSoc,wmat_pers,wcod_pret,wcod_etat_pret,wprt_dat_acc,wprt_mnt_rem,wnbr_tranche);
    }
    @GetMapping("/insertLigPret")
    void insertLigPret(@RequestParam String soc,@RequestParam String wmat_pers,@RequestParam String wcod_pret,@RequestParam String wprt_dat_deb,
                       @RequestParam String wprt_dat_fin,@RequestParam BigDecimal wprt_interet,@RequestParam BigDecimal wprt_ech,@RequestParam BigDecimal wprt_mnt_rem,
                       @RequestParam BigDecimal wrem_men,@RequestParam BigDecimal wdern_rem_men)
    {
        creditService.insert_lig_pret(soc,wmat_pers,wcod_pret,wprt_dat_deb,wprt_dat_fin,wprt_interet,wprt_ech,wprt_mnt_rem,wrem_men,wdern_rem_men);
    }
    ...
    LigPret reglerLigPret(@RequestParam String soc,@RequestParam String mat,@RequestParam String codPret,@RequestParam String lpret,@PathVariable String reg)
    {
    ...
    public List<DiskPret> getDiskPret(
        ...
        System.out.println(mat + " " + corps + " " + mois + " " + codPret);

        return diskPretRepository.getDiskPret(mat, corps, mois, codPret);
    }

    @GetMapping("/maj_lig_pret")
    void maj_lig_pret(@RequestParam String wcodSoc, @RequestParam String wmatPers, @RequestParam String wcodGrpPret, @RequestParam String corps,@RequestParam String mois)
    {
    ...
    public ResponseEntity<FileSystemResource> downloadFile(@RequestParam(defaultValue = "y:/vir/virements.txt") String filePath) {
        // Générer le fichier
...
//    DaDocFournisseur addValidDocFourn(@RequestParam String mat, @RequestParam String refDa,@RequestParam String codEtape,@RequestParam String codTypDoc,@PathVariable String valid)
//    {
//        DaDocFournisseur daDocEtape = daDocFournisseur.getByMat(mat,refDa,codEtape,codTypDoc);
...
//    }
    ...
    public ResponseEntity<List<PretPErsProjection>> getPretEnCoursLov(
        ...
        List<PretPErsProjection> result = pretPersRepository.getPretEnCoursLOV(soc, mat, nom, numComm, etat, codPret,modRemb,datSaisie,numRetr);
