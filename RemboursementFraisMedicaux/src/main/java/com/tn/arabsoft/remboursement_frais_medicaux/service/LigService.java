...
public class LigService {
    ...
    public void saveComplet(
        ...
        if (bord != null)
            bordArriverRepository.save(bord);
        if (bulletins != null && !bulletins.isEmpty())
            bultArriverRepository.saveAll(bulletins);
        if (actes != null && !actes.isEmpty())
            actes.forEach(this::saveOrUpdateActe);
        if (visites != null && !visites.isEmpty())
            visites.forEach(this::saveOrUpdateVisite);
        if (appareils != null && !appareils.isEmpty())
            appareils.forEach(this::saveOrUpdateAppareil);
        if (medicaments != null && !medicaments.isEmpty())
            medicaments.forEach(this::saveOrUpdateMedicament);
    }
    ...
    private void saveOrUpdateActe(LigActArriver acte) {
        ...
        if (existing.isPresent()) {
            ...
            if (acte.getNum_lig() == null) {
                Integer maxNum = ligActArriverRepository.findMaxNumLig(acte.getCod_soc(), acte.getMat_pers(),
                        acte.getNum_fam(), acte.getDat_soin());
    ...
    private void saveOrUpdateVisite(LigVisitArriver visite) {
        ...
        if (existing.isPresent()) {
            ...
            if (visite.getNum_lig() == null) {
                Long maxNum = ligVisitArriverRepository.findMaxNumLig(visite.getCod_soc(), visite.getMat_pers(),
                        visite.getNum_fam(), visite.getDat_soin());
    ...
    private void saveOrUpdateAppareil(LigAppArriver appareil) {
        ...
        if (existing.isPresent()) {
            ...
            if (appareil.getNum_lig() == null) {
                Integer maxNum = ligAppArriverRepository.findMaxNumLig(appareil.getCod_soc(), appareil.getMat_pers(),
                        appareil.getNum_fam(), appareil.getDat_soin());
    ...
    private void saveOrUpdateMedicament(LigMedArriver med) {
        ...
        if (existing.isPresent()) {
            ...
            if (med.getNum_lig() == null) {
                Integer maxNum = ligMedArriverRepository.findMaxNumLig(med.getCod_soc(), med.getMat_pers(),
                        med.getNum_fam(), med.getDat_soin());
