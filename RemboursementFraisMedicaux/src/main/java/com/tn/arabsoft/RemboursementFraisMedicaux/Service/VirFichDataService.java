...
import com.tn.arabsoft.RemboursementFraisMedicaux.Repositories.VirFichDataRepository;
...
public class VirFichDataService {
    ...
    public Map<String, Object> downloadVirFile(Long seq) {
        ...
        for (VirFichData record : records) {
            System.out.println("Record: " + record.getId().getLigne().substring(0, Math.min(50, record.getId().getLigne().length())));
        }
        if (records.isEmpty()) {
